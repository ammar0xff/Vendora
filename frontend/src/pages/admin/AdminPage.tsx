  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 items-center mb-4">
        <span className="text-sm text-[var(--muted)] font-medium">الفترة:</span>
        <Input type="date" className="w-36 text-sm" value={from} onChange={e => setFrom(e.target.value)} />
        <span className="text-[var(--muted)] text-sm">إلى</span>
        <Input type="date" className="w-36 text-sm" value={to} onChange={e => setTo(e.target.value)} />
      </div>

      {isLoading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.from({length:8}).map((_,i) => <div key={i} className="card p-4 h-24 animate-pulse bg-[var(--surface-2)]" />)}
        </div>
      ) : (
        <>
          {/* KPIs */}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            {/* 7-day trend */}
            <div className="card">
              <h3 className="font-bold text-[var(--text)] mb-4">الإيرادات والأرباح — آخر 7 أيام</h3>
            ...
            {/* Branch comparison */}
            <div className="card">
              <h3 className="font-bold text-[var(--text)] mb-4">مقارنة الفروع — الإيرادات</h3>
              <ResponsiveContainer width="100%" height={180}>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
            ...
            <div className="card xl:col-span-1">
              <h3 className="font-bold text-[var(--text)] mb-3 text-sm">تفاصيل الفروع</h3>
              <div className="space-y-2">
                {data?.branches?.filter((b: any) => Number(b.revenue) > 0).map((b: any) => (
                  <div key={b.id} className="flex items-center justify-between p-2.5 bg-[var(--surface-2)] rounded-xl">
                    <div>
                      <p className="font-semibold text-[var(--text)] text-sm">{b.warehouse_type === 'showroom' ? '🏪' : '🏭'} {b.name}</p>
                      <p className="text-xs text-[var(--muted)]">{b.invoice_count} فاتورة · نقدي: {fmtEGP(b.cash_sales)}</p>
                    </div>
                ...
                {!data?.branches?.some((b: any) => Number(b.revenue) > 0) && (
                  <p className="text-[var(--muted)] text-xs text-center py-4">لا توجد مبيعات في هذه الفترة</p>
                )}
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {!data?.customer_debts?.length && <p className="text-[var(--muted)] text-xs text-center py-4">لا توجد مديونيات</p>}
              ...
              {data?.customer_debts?.length > 0 && (
                <div className="mt-3 pt-3 border-t border-[var(--border-faint)] flex justify-between text-sm">
            ...
            <div className="flex flex-col gap-4">
              <div className="card p-4">
                ...
                <div className="space-y-2 max-h-32 overflow-y-auto">
                  {!data?.supplier_debts?.length && <p className="text-[var(--muted)] text-xs text-center py-2">لا توجد مديونيات</p>}
              ...
              <div className="card p-4 flex-1">
                ...
                <div className="space-y-2 max-h-32 overflow-y-auto">
                  {!data?.low_stock?.length && <p className="text-[var(--muted)] text-xs text-center py-2">✅ المخزون بخير</p>}
                  {data?.low_stock?.map((p: any) => (
                    <div key={p.name} className="flex justify-between py-1.5 border-b border-slate-50 last:border-0">
                      <span className="text-xs font-medium text-[var(--text)] truncate flex-1">{p.name}</span>
          ...
          {/* Stock per warehouse */}
          <div className="card">
            <h3 className="font-bold text-[var(--text)] mb-4 text-sm">قيمة المخزون لكل فرع</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {data?.stock_per_warehouse?.map((w: any) => (
                <div key={w.id} className="bg-[var(--surface-2)] rounded-xl p-3 border border-[var(--border-faint)]">
                  <p className="text-xs font-bold text-[var(--muted)] mb-1">{w.warehouse_type === 'showroom' ? '🏪' : '🏭'} {w.name}</p>
                  <p className="font-black text-[var(--text)]">{fmtEGP(w.stock_cost_value)}</p>
                  <p className="text-xs text-[var(--muted)]">بيع: {fmtEGP(w.stock_retail_value)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {data?.cash_drawers?.length > 0 && (
            <div className="card">
              <h3 className="font-bold text-[var(--text)] mb-3 text-sm">💵 النقدي في الدرج — الورديات المفتوحة</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
