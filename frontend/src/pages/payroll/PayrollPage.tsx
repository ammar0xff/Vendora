...
function CsvImportSection() {
  ...
  return (
    <div className="card mb-5">
      ...
      {preview.length > 0 && (
        <div className="mt-3">
          <p className="text-xs text-slate-400 mb-2">معاينة أول {preview.length} صف:</p>
          <div className="overflow-x-auto">
            <table className="text-xs w-full">
              <thead><tr>{Object.keys(preview[0]).map(h => <th key={h} className="px-2 py-1 text-right">{h}</th>)}</tr></thead>
              <tbody>{preview.map((row, i) => (
                <tr key={i}>{Object.values(row).map((v: any, j) => <td key={j} className="px-2 py-1">{v}</td>)}</tr>
              ))}</tbody>
            </table>
          </div>
        </div>
      ...
      {result && (
        <div className="mt-3 flex gap-3 text-sm">
          <span className="badge-green">+{result.added} جديد</span>
          <span className="badge-blue">~{result.updated} محدّث</span>
          <span className="badge-gray">{result.skipped} متخطى</span>
        ...
        <div>
          <div className="flex items-center gap-3 mb-5 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              <span className={
                payrollPeriod?.status === 'paid' ? 'badge-green' :
                payrollPeriod?.status === 'approved' ? 'badge-blue' :
                payrollPeriod?.status === 'review' ? 'badge-yellow' : 'badge-gray'
              }>
              ...
              {payrollPeriod?.status === 'draft' && (
                <button onClick={() => submitPeriodMut.mutate()} disabled={submitPeriodMut.isPending}
              ...
              {payrollPeriod?.status === 'review' && (
                <button onClick={() => approvePeriodMut.mutate()} disabled={approvePeriodMut.isPending}
                  className="px-3 py-2 rounded-xl text-xs font-bold text-white disabled:opacity-50" style={{ background: '#2563eb' }}>
                  اعتماد
                </button>
              )}
              {payrollPeriod?.status === 'approved' && (
                <button onClick={() => payPeriodMut.mutate()} disabled={payPeriodMut.isPending}
                  className="px-3 py-2 rounded-xl text-xs font-bold text-white disabled:opacity-50" style={{ background: '#16a34a' }}>
                  تعليم كـ مدفوع
                </button>
              )}
              {(payrollPeriod?.status === 'review' || payrollPeriod?.status === 'approved') && (
                <button onClick={() => reopenPeriodMut.mutate()} disabled={reopenPeriodMut.isPending}
                  className="px-3 py-2 rounded-xl text-xs font-bold border border-red-200 text-red-600 bg-red-50 hover:bg-red-100 disabled:opacity-50">
                  إعادة فتح
                </button>
              )}
            </div>
            <button onClick={() => calcMut.mutate()} disabled={calcMut.isPending || payrollPeriod?.status !== 'draft'}
            ...
            columns={[
              ...
              { key: 'actions', label: '', render: (e: any) => (
                <div className="flex gap-1 justify-end">
                  ...
                  <button onClick={() => setConfirmDelEmp(e.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-slate-300 hover:text-red-500"><Trash2 size={14} /></button>
                </div>
              )},
            ]}
