...
import { Plus, Trash2, CheckCircle, Package, Printer } from 'lucide-react'
...
const STATUS_CLASS: Record<string, string> = { draft: 'yellow', received: 'green', cancelled: 'red' }
...
function NewPOForm({ onClose }: { onClose: () => void }) {
    ...
    <form onSubmit={submit} className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-sm font-medium text-[var(--text-soft)]">الأصناف</label>
          <Button variant="ghost" size="sm" type="button" onClick={() => setItems(p => [...p, { product_id: '', product_name: '', qty: 1, unit_cost: 0 }])}
            className="text-blue-600 gap-1"><Plus size={12} /> إضافة صنف</Button>
        </div>
        <div className="space-y-2 max-h-72 overflow-y-auto overflow-x-visible">
          {/* Column headers */}
          <div className="grid grid-cols-12 gap-2 px-0.5">
            <div className="col-span-5 text-xs font-bold text-[var(--muted)]">الصنف</div>
            <div className="col-span-2 text-xs font-bold text-[var(--muted)] text-center">الكمية</div>
            <div className="col-span-3 text-xs font-bold text-[var(--muted)] text-center">سعر الشراء</div>
            <div className="col-span-1 text-xs font-bold text-[var(--muted)] text-left">الإجمالي</div>
          </div>
          {items.map((item, i) => (
            <div key={i} className="grid grid-cols-12 gap-2 items-start">
              {/* Product search */}
              <div className="col-span-5" style={{ position: 'relative' }}>
                <Input className={"text-sm " + (productSearch[i] && !item.product_id ? 'border-red-400 bg-red-50' : '')} placeholder="ابحث عن منتج..."
                ...
                {dropdownPos?.idx === i && productSearch[i] && !item.product_id && (
                  <div style={{ position: 'fixed', top: dropdownPos.top, left: dropdownPos.left, width: dropdownPos.width, zIndex: 99999 }}
                    className="bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-xl max-h-52 overflow-y-auto">
                    {filteredProducts(productSearch[i]).map((p: any) => (
                    <Button key={p.id} variant="ghost" type="button" onClick={() => selectProduct(i, p)}
                        className="w-full text-right justify-between px-3 py-2.5 text-sm border-b border-[var(--border-faint)] last:border-0 h-auto">
                        <div>
                          <span className="font-semibold text-[var(--text)]">{p.name}</span>
                          {p.barcode && <span className="text-[var(--muted)] text-xs mr-2 font-mono">{p.barcode}</span>}
                        </div>
                        <span className="text-[var(--muted)] text-xs flex-shrink-0 mr-2">{p.cost_price} ج.م</span>
                      </Button>
                    ))}
                    {/* Always show add-new at bottom */}
                    <Button key="add-new" variant="ghost" type="button"
                      onClick={() => { setNewProductIdx(i); setNewProduct({ name: productSearch[i], unit: 'عدد', cost_price: Number(item.unit_cost) || 0, retail_price: 0 }) }}
                      className="w-full text-right justify-start gap-2 text-sm border-t border-[var(--border-faint)] text-green-600 font-bold h-auto hover:bg-green-50">
                      <Plus size={13} />
 ...
 <Input type="number" className="text-sm" placeholder="الكمية" min="0.001" step="any"
                  value={item.qty} onChange={e => setItem(i, 'qty', e.target.value)} />
              </div>
              <div className="col-span-3">
 <Input type="number" className="text-sm" placeholder="سعر الشراء" min="0" step="0.01"
                  value={item.unit_cost} onChange={e => setItem(i, 'unit_cost', e.target.value)} />
              </div>
              <div className="col-span-1 flex items-center justify-center pt-2">
                <span className="text-xs text-[var(--muted)]">{(Number(item.qty) * Number(item.unit_cost)).toLocaleString('ar-EG')}</span>
              </div>
              <div className="col-span-1 flex items-center justify-center pt-1">
                <Button type="button" variant="ghost" size="icon-sm" onClick={() => setItems(p => p.filter((_, idx) => idx !== i))}
                  className="hover:bg-red-50 text-[var(--faint)] hover:text-red-500"><Trash2 size={13} /></Button>
              </div>
            </div>
          ))}
        </div>
        <div className="flex justify-end mt-3 pt-3 border-t border-[var(--border-faint)]">
          <span className="font-bold text-[var(--text)]">الإجمالي: <span className="text-[var(--primary)]">{total.toLocaleString('ar-EG')} ج.م</span></span>
        </div>
      </div>
 ...
 <Input type="number" value={amountPaid} onChange={e => setAmountPaid(e.target.value)} min="0" step="0.01" placeholder="0.00"/>
        </div>
        <div>
          <label className="block text-sm font-medium text-[var(--text-soft)] mb-1">استلم البضاعة</label>
 <Input value={receivedByName} onChange={e => setReceivedByName(e.target.value)} placeholder="اسم المستلم"/>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2 max-h-72 overflow-y-auto">
        <div className="grid grid-cols-12 gap-2 text-xs font-bold text-[var(--muted)] px-1 mb-1">
        ...
        {po?.items?.map((it: any) => (
          <div key={it.product_id} className="grid grid-cols-12 gap-2 items-center bg-[var(--surface-2)] rounded-xl p-2">
            <div className="col-span-5">
              <p className="font-semibold text-[var(--text)] text-sm">{it.product_name}</p>
              {it.current_cost !== it.unit_cost && (
                <p className="text-xs text-amber-600">تكلفة حالية: {it.current_cost} ج.م</p>
              )}
            </div>
            <div className="col-span-2">
 <Input type="number" className="text-sm py-1.5" min="0" step="any"
                value={getVal(it.product_id, 'qty_received', it.qty_ordered)}
                onChange={e => setVal(it.product_id, 'qty_received', Number(e.target.value))} />
            </div>
            <div className="col-span-3">
 <Input type="number" className="text-sm py-1.5" min="0" step="0.01"
                value={getVal(it.product_id, 'unit_cost', it.unit_cost)}
                onChange={e => setVal(it.product_id, 'unit_cost', Number(e.target.value))} />
            </div>
            <div className="col-span-2 text-left text-sm font-bold text-[var(--text)]">
              {(getVal(it.product_id, 'qty_received', it.qty_ordered) * getVal(it.product_id, 'unit_cost', it.unit_cost)).toLocaleString('ar-EG')}
            </div>
          </div>
        ))}
      </div>
      <div className="flex items-center justify-between pt-3 border-t border-[var(--border-faint)]">
        <div className="space-y-1">
          <div className="flex gap-4 text-sm">
            <span className="text-[var(--muted)]">الإجمالي: <span className="font-black text-[var(--primary)]">{total.toLocaleString('ar-EG')} ج.م</span></span>
          ...
          {po?.received_by_name && <p className="text-xs text-[var(--muted)]">المستلم: {po.received_by_name}</p>}
        </div>
        <div className="flex gap-3">
          <Button variant="ghost" onClick={onClose}>إلغاء</Button>
          <Button disabled={receiveMut.isPending}
...
export default function PurchasesPage() {
  ...
  const columns = [
    { key: 'po_number', label: 'رقم الأمر', render: (r: any) => <span className="font-mono font-bold text-[var(--text)]">{r.po_number}</span> },
    { key: 'supplier_name', label: 'المورد', render: (r: any) => <span className="text-[var(--text-soft)]">{r.supplier_name || '—'}</span> },
    { key: 'warehouse_name', label: 'المخزن', render: (r: any) => <span className="text-[var(--text-soft)]">{r.warehouse_name}</span> },
    { key: 'status', label: 'الحالة', render: (r: any) => <Badge variant={STATUS_CLASS[r.status] as any}>{STATUS_LABEL[r.status]}</Badge> },
    ...
    { key: 'created_at', label: 'التاريخ', render: (r: any) => <span className="text-[var(--muted)] text-sm">{new Date(r.created_at).toLocaleDateString('ar-EG')}</span> },
    {
      key: 'actions', label: '', render: (r: any) => (
        <div className="flex gap-1 justify-end items-center">
          {/* Invoice image upload */}
          <label className="p-1.5 rounded-lg hover:bg-[var(--surface-3)] text-[var(--faint)] hover:text-[var(--text-soft)] cursor-pointer" title="رفع صورة الفاتورة">
            📎
            <input type="file" accept="image/*" className="hidden" aria-label="رفع صورة الفاتورة" onChange={async e => {
          ...
          <Button variant="ghost" size="icon-sm" onClick={() => { setPrintingId(r.id); openPrint(`/print/purchase/${r.id}`); setTimeout(() => setPrintingId(null), 2000) }}
            disabled={printingId === r.id}
            className="text-[var(--faint)] hover:text-blue-600 hover:bg-blue-50 disabled:opacity-40" title="طباعة">
            <Printer size={14} />
          </Button>
          {r.status === 'draft' && (
            <Button size="sm" onClick={() => setReceivePO(r.id)}>
  ...
  return (
    <div>
      ...
      <div className="mb-4">
 <Input className="max-w-xs" placeholder="بحث برقم الأمر أو المورد..." value={search} onChange={e => setSearch(e.target.value)}/>
      </div>
