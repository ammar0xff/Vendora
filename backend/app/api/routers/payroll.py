...
async def list_employees(db: AsyncSession = Depends(get_db), _=Depends(require_role("admin"))):
...
@router.get("/periods")
async def list_periods(db: AsyncSession = Depends(get_db), _=Depends(require_role("admin"))):
    return (await db.execute(select(PayrollPeriod).order_by(PayrollPeriod.year.desc(), PayrollPeriod.month.desc()))).scalars().all()


@router.post("/periods")
async def create_period(data: dict, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_role("admin"))):
...
async def list_entries(period_id: uuid.UUID, db: AsyncSession = Depends(get_db), _=Depends(require_role("admin"))):
...
async def approve_period(period_id: uuid.UUID, db: AsyncSession = Depends(get_db), _=Depends(require_role("admin"))):
