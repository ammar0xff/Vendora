...
WHERE company = '{CAT}'
  AND subcategory_id = (SELECT id FROM subcategories WHERE name = '{cat_esc}' 
                        AND category_id = (SELECT id FROM categories WHERE name = 'الشريف'))
  AND retail_price = {price_v}
  AND size = '{size_esc}'
  AND name LIKE '%{size_esc}';""")

output = r'C:\eg-co-erp\fix_sharif_names2.sql'
with open(output, 'w', encoding='utf-8') as f:
    f.write('\n'.join(lines))

total = len(lines)
print(f"Wrote {total} UPDATE statements")
